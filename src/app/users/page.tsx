...
export default async function UsersPage() {
  ...
  return (
    <main className="flex flex-col p-8 min-h-screen">
        ...
        {users.map((user)=>(
            <div key={user.id} className="w-1/2 mx-auto flex justify-between gap-2 py-2 px-4 bg-slate-200/70 border border-slate-400/40 rounded-md shadow-md">
                <div className="flex flex-col gap-2">
                    <span className="uppercase font-semibold">Nome Completo</span>
                    <span>{user.name}</span>
                </div>
                <div className="flex flex-col gap-2">
                    <span className="uppercase font-semibold">Apelido</span>
                    <span>{user.prefered_name}</span>
                </div>
